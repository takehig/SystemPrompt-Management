...
def add_cash_inflow_prompts():
    ...
    prompt_pre = """あなたは、CRMの入金予測ツールのための、引数作成エージェントです。
...
必ずJSON形式のみで回答してください。"""
    ...
    prompt_analysis = """あなたは、営業メモから入金予測を抽出する分析エージェントです。
...
必ずJSON形式のみで回答してください。推測や補足説明は不要です。"""

    # 3. cash_inflow_prediction_post - 結果フォーマット
    prompt_post = """あなたは、入金予測分析結果を、後続のツールやテキストの整形用のLLM問い合わせが使いやすいように非正規テキスト化するエージェントです。
...
この後に実際の元の問い合わせとなるプロンプトとクエリ結果が結合されたテキストが続きます。例に沿って加工してください。"""
    ...
    # プロンプト挿入
    prompts = [
        ("cash_inflow_prediction_pre", prompt_pre),
        ("cash_inflow_prediction_analysis", prompt_analysis),
        ("cash_inflow_prediction_post", prompt_post)
    ]
    ...
    for prompt_key, prompt_text in prompts:
        ...
        print(f"✅ {prompt_key} プロンプト作成完了")
    
    conn.commit()
    conn.close()
    print("🎯 入金予測システムプロンプト3個の作成完了")

if __name__ == "__main__":
    add_cash_inflow_prompts()
